		...
		if(btn === 1) {
			...
			if(expands) {
				...
				const container = this.props.wdaggr
					.workspaceData.architecture as any;
...
export class CallGraphSpace extends 
	...
	traverseGraph(graph: NoArchCallGraph, 
		      rootIdx: string)
		: CGTreeDisplayData {
	...
	identifyRoots(graph: NoArchCallGraph): Array<[string, 
			      NoArchCallGraphEntry]> {
		
		let rootList: Array<[string, NoArchCallGraphEntry]> = [];
