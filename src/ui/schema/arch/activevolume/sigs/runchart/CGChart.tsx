...
import { CallGraphStatsData, CUAggrKey, CUDataKeyRef, CUScaleKeyRef, 
	DataAggregate, DataAggrIdentifier, DataAggrMap } from "./ChartData.ts";
...
export const CallGraphStatsSpace = (props: CallGraphStatsData) => {
	...
	const nCols: Array<string> = data.idxs
		.map((_, idx) => CircleColorList[idx % CircleColorList.length]!)
	const nLins: Array<string> = data.idxs.map((_, idx) => LineColorList[idx % 
	...
	const chartRef = useRef(new SVGSVGElement()) as React.MutableRefObject<SVGSVGElement>;
